...
con <- DBI::dbConnect(
  ...
  dbname = "MarketingAnalyticsClassData",
  host = "byumarketinganalytics.chaby6kshgs3.us-east-2.rds.amazonaws.com",
  port = 5555,
  user = "adrielc",
  password = "byuanalytics58"
)

# List data tables in the connection.
DBI::dbListTables(con)

# Write data tables to the database.
DBI::dbWriteTable(con, "store_data", store_data)
DBI::dbWriteTable(con, "online_data", online_data)
DBI::dbWriteTable(con, "sat_data", sat_data)
